...
//#region src/models/base_model.ts
var BaseModel = class {
	constructor(params) {
...
//#endregion
//#region src/models/api_error.ts
/**
...
//#endregion
//#region src/http_client/base.ts
/**
...
*/
var ApiRequest = class ApiRequest {
	/**
	...
	*/
	static async create(uri, method, body, params, clientData) {
		...
		apiRequest.response = await apiRequest.createPromise(uri, method, body, clientData);
		return apiRequest;
	}
	/**
	...
	*/
	async createPromise(uri, method, body, clientData) {
		...
		const options = {
			method,
			headers: await this.buildHeaders(clientData, method, body),
			...method !== "GET" && body ? { body: JSON.stringify(body) } : {}
		};
		const target = new URL(url, prefixUrl);
		const stringifiedParams = Object.fromEntries(Object.entries(this.params).filter(([, value]) => value !== void 0 && value !== null).map(([key, value]) => [key, String(value)]));
		target.search = new URLSearchParams(stringifiedParams).toString();
		return this.fetchAndHandleResponse(target, options, clientData.requestTimeout);
	}
	/**
	...
	async fetchAndHandleResponse(target, options, requestTimeout = 0) {
		...
		try {
			...
			if (err instanceof Error) {
				if (err.name === "TimeoutError") return Promise.reject(new ApiError(`Request timed out after ${requestTimeout}ms`, 408, { reason: "timeout" }));
				return Promise.reject(new ApiError(err.message, 500, { reason: "network or fetch error" }));
			}
			return Promise.reject(new ApiError("An unknown error occurred", 500, { reason: String(err) }));
		}
	}
	/**
	...
	*/
	async buildHeaders(clientData, method, body) {
		...
		headers.append(clientData.authHeader, clientData.tokenType.length > 0 ? `${clientData.tokenType} ${clientData.token}` : clientData.token);
		if (clientData.enableCompression) headers.append("Accept-Encoding", "gzip,deflate");
		if (method !== "GET" && body) headers.append("Content-Type", "application/json");
		return headers;
	}
	/**
	...
	*/
	composeURI(rawUri) {
		const uri = rawUri.replace(/\{(!?):(\w+)\}/g, this.mapUriParams());
	...
	*/
	mapUriParams() {
		return (_substring, isMandatory, paramName) => {
			...
			if (isMandatory === "!") throw new Error(`Missing required parameter: ${paramName}`);
			return "";
		};
	}
};
...
//#endregion
//#region src/models/paginated_result.ts
var PaginatedResult = class {
	...
	safeParseInt(str) {
		if (!str || Number.isNaN(Number(str))) return 0;
		return Number.parseInt(str, 10);
	}
};

//#endregion
//#region src/models/cursor_paginated_result.ts
...
var BaseCollection = class {
	...
	*/
	get rootElementName() {
		throw new Error("rootElementName is not defined. Subclasses must override `rootElementName`.");
	}
	/**
	...
	*/
	get rootElementNameSingular() {
		throw new Error("rootElementNameSingular is not defined. Subclasses must override `rootElementNameSingular`.");
	}
	/**
	...
	*/
	get secondaryElementClass() {
		throw new Error("Secondary elements are not supported by this collection. Override `secondaryElementClass` if needed.");
	}
	/**
	...
	*/
	get secondaryElementNameSingular() {
		throw new Error("secondaryElementNameSingular is not defined. Subclasses must override this if secondary elements are used.");
	}
	/**
	...
	*/
	doGet(id, params = {}) {
		return this.createPromise("GET", {
			...params,
			id
		}, this.populateObjectFromJsonRoot, null);
	}
	/**
	...
	*/
	doDelete(id, params = {}) {
		return this.createPromise("DELETE", {
			...params,
			id
		}, this.returnBareJSON, null);
	}
	/**
	...
	*/
	doUpdate(id, body, params, resolveFn = this.populateObjectFromJsonRoot, method = "PUT") {
		return this.createPromise(method, {
			...params,
			id
		}, resolveFn, body);
	}
	...
	*/
	populateObjectFromJsonRoot(json, headers) {
		...
		if (rootElementName) {
			...
			if (!this.isRecord(picked)) throw new Error(`Missing property '${rootElementName}' in JSON object`);
			jsonData = picked;
		}
	...
	*/
	async prepareRequest(method, body, params, uri) {
		return await ApiRequest.create(this.getUri(uri), method, body, params, this.clientData);
	}
	/**
	...
	*/
	getUri(uri) {
		...
		if (!resolvedUri) throw new Error("No URI or prefixURI provided. Ensure the subclass defines a static prefixURI or pass a URI explicitly.");
		return resolvedUri;
	}
...
//#endregion
//#region src/collections/branches.ts
var Branches = class extends BaseCollection {
	...
	merge(branch_id, request_params, body = {}) {
		...
		return this.createPromise("POST", params, this.returnBareJSON, body, "projects/{!:project_id}/branches/{:id}/merge");
	}
};
...
//#endregion
//#region src/collections/comments.ts
var Comments = class extends BaseCollection {
	...
	list_project_comments(params) {
		return this.createPromise("GET", params, this.populateArrayFromJson, null, "projects/{!:project_id}/comments");
	}
};

//#endregion
//#region src/models/contributor.ts
var Contributor = class extends BaseModel {};

//#endregion
//#region src/collections/contributors.ts
...
//#endregion
//#region src/models/file.ts
var File = class extends BaseModel {};

//#endregion
//#region src/models/queued_process.ts
...
//#endregion
//#region src/collections/files.ts
var Files = class extends BaseCollection {
	...
	returnBareJSON(json, headers) {
		if (this.isResponseTooBig(headers)) warn(this.clientData.silent, "\x1B[33m\x1B[1mWarning:\x1B[0m Project too big for sync export. Please use our async export lokaliseApi.files().async_download() method.");
		return {
	...
	async_download(project_id, download) {
		return this.createPromise("POST", { project_id }, this.populateSecondaryObjectFromJson, download, "projects/{!:project_id}/files/async-download");
	}
...
//#endregion
//#region src/models/glossary_term.ts
var GlossaryTerm = class extends BaseModel {};

//#endregion
//#region src/collections/glossary_terms.ts
var GlossaryTerms = class extends BaseCollection {
	...
	create(term_params, request_params) {
		return this.createPromise("POST", request_params, this.populateArrayFromJsonBulk, term_params);
	}
	update(term_params, request_params) {
		return this.createPromise("PUT", request_params, this.populateArrayFromJsonBulk, term_params, "projects/{!:project_id}/glossary-terms");
	}
	delete(term_ids, request_params) {
		const keys = { terms: term_ids };
		return this.createPromise("DELETE", request_params, this.populateFromBulkDelete, keys, "projects/{!:project_id}/glossary-terms");
	}
...
//#endregion
//#region src/models/jwt.ts
var Jwt$1 = class extends BaseModel {};

//#endregion
//#region src/collections/jwt.ts
var Jwt = class extends BaseCollection {
...
//#endregion
//#region src/models/key.ts
var Key = class extends BaseModel {};

//#endregion
//#region src/collections/keys.ts
var Keys = class extends BaseCollection {
	...
	create(key_params, request_params) {
		return this.createPromise("POST", request_params, this.populateArrayFromJsonBulk, key_params);
	}
	...
	bulk_update(key_params, request_params) {
		return this.createPromise("PUT", request_params, this.populateArrayFromJsonBulk, key_params, "projects/{!:project_id}/keys");
	}
	bulk_delete(key_ids, request_params) {
		const keys = { keys: key_ids };
		return this.createPromise("DELETE", request_params, this.returnBareJSON, keys, "projects/{!:project_id}/keys");
	}
};

//#endregion
//#region src/models/language.ts
var Language = class extends BaseModel {};

//#endregion
//#region src/collections/languages.ts
var Languages = class extends BaseCollection {
	...
	system_languages(params = {}) {
		return this.createPromise("GET", params, this.populateArrayFromJson, null, "system/languages");
	}
	list(request_params) {
		return this.doList(request_params);
	}
	create(raw_body, request_params) {
		...
		return this.createPromise("POST", request_params, this.populateArrayFromJsonBulk, body);
	}
...
//#endregion
//#region src/models/order.ts
var Order = class extends BaseModel {};

//#endregion
//#region src/collections/orders.ts
...
//#endregion
//#region src/models/payment_card.ts
var PaymentCard = class extends BaseModel {};

//#endregion
//#region src/collections/payment_cards.ts
...
//#endregion
//#region src/models/permission_template.ts
var PermissionTemplate = class extends BaseModel {};

//#endregion
//#region src/collections/permission_templates.ts
...
var Projects = class extends BaseCollection {
	...
	empty(project_id) {
		return this.createPromise("PUT", { project_id }, this.returnBareJSON, null, "projects/{!:project_id}/empty");
	}
};

//#endregion
//#region src/collections/queued_processes.ts
...
//#endregion
//#region src/models/screenshot.ts
var Screenshot = class extends BaseModel {};

//#endregion
//#region src/collections/screenshots.ts
...
//#endregion
//#region src/models/segment.ts
var Segment = class extends BaseModel {};

//#endregion
//#region src/collections/segments.ts
...
//#endregion
//#region src/models/snapshot.ts
var Snapshot = class extends BaseModel {};

//#endregion
//#region src/collections/snapshots.ts
...
//#endregion
//#region src/models/task.ts
var Task = class extends BaseModel {};

//#endregion
//#region src/collections/tasks.ts
...
//#endregion
//#region src/models/team_user_billing_details.ts
var TeamUserBillingDetails$1 = class extends BaseModel {};

//#endregion
//#region src/collections/team_user_billing_details.ts
var TeamUserBillingDetails = class extends BaseCollection {
	static prefixURI = "teams/{!:team_id}/billing_details";
	get elementClass() {
		return TeamUserBillingDetails$1;
	}
	get(team_id) {
	...
	update(team_id, billing_details_params) {
		const params = { team_id };
		return this.createPromise("PUT", params, this.populateObjectFromJson, billing_details_params);
	}
};

//#endregion
//#region src/models/team_user.ts
var TeamUser = class extends BaseModel {};

//#endregion
//#region src/collections/team_users.ts
...
//#endregion
//#region src/models/team.ts
var Team = class extends BaseModel {};

//#endregion
//#region src/collections/teams.ts
...
//#endregion
//#region src/collections/translation_providers.ts
...
//#endregion
//#region src/models/translation_status.ts
var TranslationStatus = class extends BaseModel {};

//#endregion
//#region src/collections/translation_statuses.ts
var TranslationStatuses = class extends BaseCollection {
	...
	available_colors(request_params) {
		return this.createPromise("GET", request_params, this.returnBareJSON, {}, "projects/{!:project_id}/custom_translation_statuses/colors");
	}
};

//#endregion
//#region src/models/translation.ts
var Translation = class extends BaseModel {};

//#endregion
//#region src/collections/translations.ts
...
//#endregion
//#region src/models/user_group.ts
var UserGroup = class extends BaseModel {};

//#endregion
//#region src/collections/user_groups.ts
var UserGroups = class extends BaseCollection {
	...
	add_members_to_group(team_id, group_id, user_ids) {
		...
		return this.createPromise("PUT", params, this.populateGroupFromJsonRoot, body, "teams/{!:team_id}/groups/{!:group_id}/members/add");
	}
	remove_members_from_group(team_id, group_id, user_ids) {
		...
		const body = { users: user_ids };
		return this.createPromise("PUT", params, this.populateGroupFromJsonRoot, body, "teams/{!:team_id}/groups/{!:group_id}/members/remove");
	}
	add_projects_to_group(team_id, group_id, project_ids) {
		...
		return this.createPromise("PUT", params, this.populateGroupFromJsonRoot, body, "teams/{!:team_id}/groups/{!:group_id}/projects/add");
	}
	remove_projects_from_group(team_id, group_id, project_ids) {
		...
		const body = { projects: project_ids };
		return this.createPromise("PUT", params, this.populateGroupFromJsonRoot, body, "teams/{!:team_id}/groups/{!:group_id}/projects/remove");
	}
...
//#endregion
//#region src/collections/webhooks.ts
var Webhooks = class extends BaseCollection {
	...
	regenerate_secret(webhook_id, request_params) {
		...
		return this.createPromise("PATCH", params, this.returnBareJSON, null, "projects/{!:project_id}/webhooks/{:id}/secret/regenerate");
	}
};

//#endregion
//#region src/lokalise/base_client.ts
/**
...
var BaseClient = class {
	...
	*/
	constructor({ apiKey, enableCompression = false, silent = false, tokenType = "", host, requestTimeout, userAgent }) {
		if (typeof apiKey !== "string" || apiKey.trim().length === 0) throw new Error("Instantiation failed: A non-empty API key or JWT must be provided.");
		this.clientData = {
			token: apiKey,
...
//#endregion
//#region src/lokalise/lokalise_api.ts
/**
...
//#endregion
//#region src/lokalise/lokalise_api_oauth.ts
/**
...
//#endregion
//#region src/models/ota/ota_bundle.ts
var OtaBundle = class extends BaseModel {};

//#endregion
//#region src/ota_collections/ota_collection.ts
var OtaCollection = class extends BaseCollection {
	doDelete(id, req_params) {
		...
		return this.createPromise("DELETE", params, this.returnJSONFromData, null);
	}
	returnJSONFromData(json) {
		const data = json.data;
		if (data && typeof data === "object" && (!Array.isArray(data) || data.every((item) => typeof item === "object" && item !== null))) return data;
		throw new Error("Invalid response format: expected object or array of objects in `data`");
	}
...
//#endregion
//#region src/ota_collections/ota_bundle_management.ts
...
//#endregion
//#region src/ota_collections/ota_bundle_publishing.ts
var OtaBundlePublishing = class extends OtaCollection {
	...
	publish(bundleId, request_params) {
		...
		return this.createVoidPromise("POST", params, { bundleId });
	}
...
//#endregion
//#region src/models/ota/ota_bundle_archive.ts
var OtaBundleArchive = class extends BaseModel {};

//#endregion
//#region src/ota_collections/ota_bundles.ts
var OtaBundles = class extends OtaCollection {
	...
	get(bundle_params, request_params) {
		...
		return this.createPromise("GET", params, this.populateObjectFromJsonRoot, null);
	}
};

//#endregion
//#region src/models/ota/ota_freeze_period.ts
var OtaFreezePeriod = class extends BaseModel {};

//#endregion
//#region src/ota_collections/ota_freeze_periods.ts
...
//#endregion
//#region src/models/ota/ota_sdk_token.ts
var OtaSdkToken = class extends BaseModel {};

//#endregion
//#region src/ota_collections/ota_sdk_tokens.ts
...
//#endregion
//#region src/models/ota/ota_statistics.ts
var OtaStatistics = class extends BaseModel {};

//#endregion
//#region src/ota_collections/ota_usage_statistics.ts
...
//#endregion
//#region src/lokalise/lokalise_api_ota.ts
/**
...
//#endregion
//#region src/oauth2/auth_request.ts
/**
...
async function createPromise(uri, method, body, { host, version }) {
	...
	return fetchAndHandleResponse(new URL(fullUri, host), {
		method,
...
//#endregion
//#region src/oauth2/lokalise_auth.ts
var LokaliseAuth = class {
	...
	*/
	constructor(clientId, clientSecret, host = "https://app.lokalise.com", version = "oauth2") {
		if (!clientId || !clientSecret) throw new Error("Error: Instantiation failed: Please pass client ID and client secret");
		this.authData = {
			client_id: clientId,
	...
	*/
	async doRequest(params) {
		try {
			return (await createPromise("token", "POST", params, this.authData)).json;
		} catch (err) {
...
//#endregion
//#region src/models/auth_error.ts
